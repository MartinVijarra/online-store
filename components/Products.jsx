...
export default function Products() {
  //Api meli 'https://api.mercadolibre.com/sites/MLA/search?seller_id=179571326&limit=15'
  ...
  return (
    <section className="w-full py-24 bg-gray-100">
      <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4">
        ...
        {/* Mapping de datos API   */}
        
        {results?.map((product) => (
          <Link
            href={"/ItemDetail"}
            className="w-64 h-full flex m-auto items-center bg-white rounded-xl hover:shadow-xl"
            key={product.id}
          >
            <div className="w-full h-full py-8 flex flex-col justify-center">
              <img
                src={product.thumbnail}
                className="p-5 md:py-4 cursor-pointer"
                alt={product.title}
              />
